<?php
	...
	if($result->num_rows > 0){ /*no error*/
		if($table=="MORRO"){
			...
			$res="<center><table class='table' style='background-color: white; border-radius: 1%; width: 88%'> <thead> <th> FOLIO </th> <th scope='col'> NOMBRE </th><th scope='col'><center> FOTO </center></th><th scope='col'> DIRECCION </th><th scope='col'> OCUPACION </th><th scope='col'> PLAZA </th>  <th scope='col'> SUELDO </th> <th scope='col'> NUMERO DE EMPLEADO </th> <th scope='col'> ADSCRIPCION </th><th scope='col'> JEFE </th><th scope='col'> HORARIO </th><th scope='col'> TELEFONO CELULAR</th><th scope='col'> TELEFONO FIJO </th><th scope='col'> CURP </th></thead>";
			while($row = $result->fetch_assoc()) {
			$res=$res."<tr>
	    		<th scope=row><center> ". $row["MORRO_Boleta_D"]." </center></td>
	    		<td><center> " . $row["NOM_D"]." ".$row["APE_PAT_D"]." ".$row["APE_MAT_D"]. " </center></td>
	    		<td><center><img src='img/" . $row["FOTO_D"]. "' width=75%/></center></td>
	    	...
	    	$res= "<center><table class='table' style='background-color: white; border-radius: 1%; width: 88%'> <thead> <th> FOLIO </th> <th scope='col'> NOMBRE </th><th scope='col'><center> FOTO </center></th><th scope='col'> DIRECCION </th><th scope='col'> LUGAR DE TRABAJO </th>  <th scope='col'> TELEFONO TRABAJO </th> <th scope='col'> TELEFONO CASA </th> <th scope='col'> RELIGION </th><th scope='col'> CURP </th></thead>";
	    	while($row = $result->fetch_assoc()) {
			$res=$res."<tr>
	    		<th scope=row><center> ". $row["MORRO_Boleta_C"]." </center></td>
	    		<td><center> " . $row["NOM_C"]." ".$row["APE_PAT_C"]." ".$row["APE_MAT_C"]. " </center></td>
	    		<td><center><img src='img/" . $row["FOTO_C"]. "' width=75%/></center></td>
	    		...
	    		<td><center>" . $row["CURP_C"] . " </center></td>
	    	  </tr>";
	  		}    	/**/
	    }
